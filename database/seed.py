...
from database.models import Diet, Cow, Employee, Section, Camera, Notification, AIActivity
...
def seed():
    ...
    try:
        ...
        activities_data = [
            ...
            # А-03: только сон (стельная, мало активности)
            ...
            # Б-02: мало ест (стельная, снижение BCS)
            ...
            # Б-03: аномальный сон (не покидает зону отдыха)
            ...
            # В-01: на лечении, минимальная активность
        ...
        # ── Уведомления (история) ──
        notifications_data = [
            (cow_map["В-01"].id_cow, "ACTIVITY", "CRITICAL",
             "Обнаружено падение животного в карантинном боксе.",
             now - timedelta(hours=18)),
            (cow_map["В-01"].id_cow, "ACTIVITY", "WARNING",
             "Низкая двигательная активность в течение 6 часов.",
             now - timedelta(hours=14)),
            (cow_map["А-03"].id_cow, "FEED", "WARNING",
             "Отказ от корма: животное не подходило к кормушке более 8 часов.",
             now - timedelta(hours=10)),
            (cow_map["Б-02"].id_cow, "BCS", "WARNING",
             "Снижение оценки BCS до 2.1 балла (норма 2.5–3.5).",
             now - timedelta(hours=8)),
            (cow_map["А-01"].id_cow, "SLEEP", "WARNING",
             "Продолжительность лежания 16.2 ч/сут (норма 10–14 ч).",
             now - timedelta(hours=5)),
            (cow_map["В-02"].id_cow, "ACTIVITY", "WARNING",
             "Снижение потребления воды: 0 подходов к поилке за 12 часов.",
             now - timedelta(hours=3)),
            (cow_map["Б-03"].id_cow, "FEED", "CRITICAL",
             "Аномальное поведение: животное не покидает зону отдыха более 24 часов.",
